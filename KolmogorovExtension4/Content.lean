...
theorem le_sum_of_additive_aux {J : Finset (Set α)} (h_ss : ↑J ⊆ C) (h_mem : ⋃₀ ↑J ∈ C) :
    m (⋃₀ ↑J) ≤ ∑ u in J, m u := by
  classical
  rw [← hC.sUnion_allDiffFinset₀ _ h_ss, m_add]
  rotate_left
  · exact hC.allDiffFinset₀_subset _ h_ss
  · exact hC.pairwiseDisjoint_allDiffFinset₀ _ h_ss
  · rwa [hC.sUnion_allDiffFinset₀ _ h_ss]
  ...
  · exact hC.indexedDiffFinset₀_subset _ h_ss i
  · exact hC.pairwiseDisjoint_indexedDiffFinset₀' _ h_ss i
  · exact h_ss (ordered_mem i)
  · exact fun _ ↦ hC.subset_ordered_of_mem_indexedDiffFinset₀ _ _
