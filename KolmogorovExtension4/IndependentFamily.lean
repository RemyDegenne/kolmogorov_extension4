...
lemma rightinverse_of_proj [hα : ∀ i, Nonempty (α i)] (I : Set ι) :
    ∃ f : ((i : I) → α i) → ((i : ι) → α i), Function.projI α I ∘ f = id := by
  classical
  use (fun (t : (i : I) → α i) (i : ι) =>
    dite (i ∈ I) (fun h => t ⟨i, h⟩) (fun _ => ((hα i).some : (α i))))
  simp [Function.projI]
  ext x i
...
def boxIJ {I J : Set ι} (hJI : J ⊆ I) (t : (i : ι) → Set (α i)) : Set ((i : I) → α i) :=
  (fun (f : (i : I) → α i) (i : J) => f ⟨i, hJI i.prop⟩)⁻¹' boxI J t

example (A : Set ι) : A ⊆ univ := subset_univ A

lemma projI_mem_boxIJ {I J : Set ι} (hJI : J ⊆ I) (t : (i : ι) → Set (α i)) (x : (i : ι) → α i) :
    Function.projI α I x ∈ boxIJ hJI t ↔ (∀ i ∈ J, x i ∈ t i) := by
...
theorem Measure.subset_pi_eval_boxI' [DecidableEq ι] (I J : Finset ι) (hJI : J ⊆ I)
    (P : ∀ i : ι, Measure (α i)) [∀ i : ι, IsProbabilityMeasure (P i)] (t : (i : ι) → Set (α i)) :
...
theorem proj' [DecidableEq ι] (P : ∀ i, Measure (α i)) [∀ i, IsProbabilityMeasure (P i)]
  ...
  · intros s hs
    ...
    rw [ht1]
    classical
    rw [preimage_boxI' hJI t, Measure.subset_pi_eval_boxI, Measure.subset_pi_eval_boxI']
    exact hJI
  · let t := fun (i : ι) => (univ : Set (α i))
    ...
    simp only [Measure.subset_pi_eval_boxI, Finset.univ_eq_attach, measure_univ,
      Finset.prod_const_one, dite_eq_ite, ite_self]
    change univ ∈ boxesI J C
    rw [mem_boxesI univ C hu]
    use t, boxI_univ (J : Set ι)
...
noncomputable def independentFamily [∀ i, PseudoEMetricSpace (α i)]
    ...
    Measure (∀ i, α i) :=
  projectiveLimitWithWeakestHypotheses (fun J : Finset ι => Measure.subset_pi P J)
