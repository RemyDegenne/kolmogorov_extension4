...
theorem eq_ionescuTulceaKernel {a : ℕ} (η : kernel ((i : Iic a) → X i) ((n : ℕ) → X n))
  ...
  rw [isProjectiveLimit_nat_iff]
  · intro n
    rw [inducedFamily_Iic, ← kernel.map_apply _ (meas_proj n), hη n]
  · exact (isProjectiveMeasureFamily_inducedFamily _ (partialKernel_proj_apply κ x₀))
...
theorem ionescuTulceaKernel_eq (n : ℕ) :
  ...
  have _ I : IsFiniteMeasure (inducedFamily (fun k ↦ partialKernel κ n k x) I) := by
    rw [inducedFamily]
    infer_instance
  refine isProjectiveLimit_unique (isProjectiveLimit_ionescuTulceaFun _ _ _) ?_
  rw [isProjectiveLimit_nat_iff' _ (isProjectiveMeasureFamily_inducedFamily _ (partialKernel_proj_apply κ x)) _ (n + 1)]
  intro k hk
...
theorem victoire {a b : ℕ} (hab : a ≤ b) : (partialKernel κ a b) ⊗ₖ
  ...
  rw [kernel.compProd_apply _ _ _ ms, kernel.map_apply' _ _ _ ms, ← partialKernel_comp_ionescuTulceaKernel κ hab,
    kernel.comp_apply']
  · congr with y
    rw [kernel.comap_apply, ionescuTulceaKernel_eq, kernel.map_apply', kernel.prod_apply',
...
theorem integral_ionescuTulceaKernel {n : ℕ} (x₀ : (i : Iic n) → X i) {f : ((n : ℕ) → X n) → E}
  ...
  rw [ionescuTulceaKernel_eq, kernel.map_apply, integral_map, kernel.integral_prod,
  ...
  · change Integrable (f ∘ (fun x ↦ updateFinset x _ x₀) ∘ (el' n)) _
    ...
    · convert i_f
      ...
      · congr with x
        simp only [id_eq, Set.mem_preimage, Function.comp_apply, Set.mem_setOf_eq]
        congrm (fun i ↦ ?_) ∈ s
  ...
  · rwa [ionescuTulceaKernel_eq, kernel.map_apply, integrable_map_measure] at i_f
    · exact mf.aestronglyMeasurable
    · exact (el' n).measurable.aemeasurable
  · exact (el' n).measurable.aemeasurable
  · exact mf.aestronglyMeasurable
...
theorem partialKernel_comp_ionescuTulceaKernel_apply {a b : ℕ} (hab : a ≤ b)
  ...
  · apply integral_congr_ae
    ...
    intro x hx1 hx2
    rw [ionescuTulceaKernel_eq, kernel.map_apply, integral_map, kernel.prod_apply, MeasureTheory.integral_prod,
      kernel.integral_deterministic', integral_map, MeasureTheory.integral_prod,
      kernel.integral_deterministic']
    · congr with y
      rw [projel']
      rfl
