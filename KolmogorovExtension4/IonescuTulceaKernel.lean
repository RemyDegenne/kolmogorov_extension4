...
theorem isProjectiveLimit_nat_iff' (μ : (I : Finset ℕ) → Measure ((i : I) → X i))
  ...
  rw [← fproj₂_comp_fproj (I.sub_Iic.trans (Iic_subset_Iic.2 (le_max_left (I.sup id) a))),
    ← Measure.map_map (measurable_fproj₂ _) (measurable_fprojNat _),
...
theorem isProjectiveMeasureFamily_inducedFamily (μ : (n : ℕ) → Measure ((i : Iic n) → X i))
  ...
  rw [Measure.map_map]
  conv_rhs => enter [1]; change fproj₂ (hJI.trans I.sub_Iic)
  rw [← fproj₂_comp_fproj₂ J.sub_Iic (Iic_subset_Iic.2 sls), ← Measure.map_map,
    h (J.sup id) (I.sup id) sls]
  any_goals exact measurable_fproj₂ _
  exact measurable_fproj₂ hJI
