...
#align_import caratheodory_extension
...
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (s «expr ∉ » C) -/
...
theorem ofFunction_eq_iInf_mem (s : Set α) (m : Set α → ℝ≥0∞) (m_empty : m ∅ = 0)
    ...
    OuterMeasure.ofFunction m m_empty s =
      ⨅ (f : ℕ → Set α) (hf : ∀ i, f i ∈ C) (h : s ⊆ ⋃ i, f i), ∑' i, m (f i) :=
  by
  rw [outer_measure.of_function_apply]
  refine' le_antisymm _ _
  · refine' le_iInf fun f => le_iInf fun hf => le_iInf fun h => _
    refine' iInf₂_le _ _
    exact h
  · simp_rw [le_iInf_iff]
    intro f hf_subset
    ...
    · simp only [hf, WithTop.iInf_empty, top_le_iff]
      push_neg at hf 
...
theorem inducedOuterMeasure_eq_iInf_mem (hC : ∅ ∈ C) (m : ∀ s : Set α, s ∈ C → ℝ≥0∞)
    ...
    inducedOuterMeasure m hC m_empty s =
      ⨅ (f : ℕ → Set α) (hf : ∀ i, f i ∈ C) (h : s ⊆ ⋃ i, f i), ∑' i, m (f i) (hf i) :=
  by
  rw [induced_outer_measure, of_function_eq_infi_mem s (extend m) _ fun s hs => extend_eq_top m hs]
  simp_rw [← extend_eq m]

/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (s «expr ∉ » C) -/
theorem OuterMeasure.ofFunction_eq_of_mono_of_subadditive (hC : SetSemiring C) (m : Set α → ℝ≥0∞)
    (m_empty : m ∅ = 0) (m_mono : ∀ ⦃s t : Set α⦄ (hs : s ∈ C) (ht : t ∈ C), s ⊆ t → m s ≤ m t)
    (m_sigma_subadd :
  ...
  by
  refine' le_antisymm (outer_measure.of_function_le s) _
  rw [of_function_eq_infi_mem s m m_empty m_top]
  refine' le_iInf fun f => le_iInf fun hf => le_iInf fun hs_subset => _
  calc
    m s = m (s ∩ ⋃ i, f i) := by rw [inter_eq_self_of_subset_left hs_subset]
    _ = m (⋃ i, s ∩ f i) := by rw [inter_Union]
    _ ≤ ∑' i, m (s ∩ f i) := by
      refine' m_sigma_subadd (fun i => _) _
      · exact hC.inter_mem _ hs _ (hf i)
      · rwa [← inter_Union, inter_eq_self_of_subset_left hs_subset]
    _ ≤ ∑' i, m (f i) :=
      by
...
theorem OuterMeasure.ofFunction_eq_of_add_of_subadditive (hC : SetSemiring C) (m : Set α → ℝ≥0∞)
    ...
    OuterMeasure.ofFunction m m_empty s = m s :=
  by
  refine' outer_measure.of_function_eq_of_mono_of_subadditive hC m m_empty _ m_sigma_subadd m_top hs
  intro t u ht hu htu
  exact monotone_of_additive hC m m_add ht hu htu
...
theorem inducedOuterMeasure_eq_of_add_of_subadditive (hC : SetSemiring C)
  ...
  (OuterMeasure.ofFunction_eq_of_add_of_subadditive hC (extend m) _ (extend_sUnion_eq_sum m m_add)
        (extend_sum_le m m_sigma_subadd) (fun s hs => extend_eq_top m hs) hs).trans
    (extend_eq m hs)

/- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (i n) -/
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (s «expr ∉ » C) -/
theorem caratheodory_semiring_extension' (hC : SetSemiring C) (m : Set α → ℝ≥0∞) (m_empty : m ∅ = 0)
    ...
    (m_sigma_subadd :
      ∀ ⦃f : ℕ → Set α⦄ (hf : ∀ i, f i ∈ C) (hf_Union : (⋃ i, f i) ∈ C),
        m (⋃ i, f i) ≤ ∑' i, m (f i))
    (m_top : ∀ (s) (_ : s ∉ C), m s = ∞) {s : Set α} (hs : s ∈ C) :
    OuterMeasure.IsCaratheodory (OuterMeasure.ofFunction m m_empty) s :=
  by
  rw [outer_measure.is_caratheodory_iff_le']
  intro t
  conv_rhs => rw [of_function_eq_infi_mem _ _ m_empty m_top]
  refine' le_iInf fun f => le_iInf fun hf => le_iInf fun hf_subset => _
  let A : ℕ → Finset (Set α) := fun i => hC.diff_finset (hC.inter_mem _ (hf i) _ hs) (hf i)
  ...
  have h_m_eq : ∀ i, m (f i) = m (f i ∩ s) + ∑ u in A i, m u :=
    by
    intro i
    rw [hC.eq_add_diff_finset_of_subset m m_add (hC.inter_mem _ (hf i) _ hs) (hf i)
    ...
    have h_sum_sum : ∑' i, ∑ u in A i, m u = ∑' n, m (g n) :=
      ...
      have h1 : ∀ i, ∑ u in A i, m u = ∑' n, m (g' ⟨i, n⟩) :=
        ...
        have :
          ...
          · simp only [h_lt, mem_set_of_eq, if_true]
          · have : ¬(i, n).snd < (A (i, n).fst).card :=
              by
              change ¬n < (A i).card
              simpa only [coe_range, Set.mem_Iio] using h_lt
            simp only [h_lt, mem_set_of_eq, if_false, this, not_false_iff, dif_neg, m_empty]
        rw [this]
...
theorem caratheodory_semiring_extension (hC : SetSemiring C) (m : ∀ s : Set α, s ∈ C → ℝ≥0∞)
    ...
    (m_sigma_subadd :
      ∀ ⦃f : ℕ → Set α⦄ (hf : ∀ i, f i ∈ C) (hf_Union : (⋃ i, f i) ∈ C),
        m (⋃ i, f i) hf_Union ≤ ∑' i, m (f i) (hf i))
    {s : Set α} (hs : s ∈ C) : (inducedOuterMeasure m hC.empty_mem m_empty).IsCaratheodory s :=
  caratheodory_semiring_extension' hC (extend m) _ (extend_sUnion_eq_sum m m_add)
    (extend_sum_le m m_sigma_subadd) (fun s hs => extend_eq_top m hs) hs
...
noncomputable def Measure.ofAddSubaddCaratheodory (hC : SetSemiring C)
  ...
  {
    inducedOuterMeasure m hC.empty_mem
      m_empty with
    m_iUnion := fun f hf hd => OuterMeasure.iUnion_eq_of_caratheodory _ hf hd
    trimmed :=
      by
      letI : MeasurableSpace α := (induced_outer_measure m hC.empty_mem m_empty).caratheodory
      show
        @outer_measure.trim _ (induced_outer_measure m hC.empty_mem m_empty).caratheodory
            (induced_outer_measure m hC.empty_mem m_empty) =
          induced_outer_measure m hC.empty_mem m_empty
      refine' le_antisymm (le_induced_outer_measure.mpr fun s hs => _) (outer_measure.le_trim _)
      have hs_meas :
        measurable_set[(induced_outer_measure m hC.empty_mem m_empty).caratheodory] s :=
        by
        show (induced_outer_measure m hC.empty_mem m_empty).IsCaratheodory s
        exact caratheodory_semiring_extension hC m m_empty m_add m_sigma_subadd hs
      rw [outer_measure.trim_eq _ hs_meas,
        induced_outer_measure_eq_of_add_of_subadditive hC m m_empty m_add m_sigma_subadd hs]
      exact le_rfl }
...
theorem isCaratheodory_partialSups {m : OuterMeasure α} {s : ℕ → Set α}
    (h : ∀ i, m.IsCaratheodory (s i)) (i : ℕ) : m.IsCaratheodory (partialSups s i) :=
  by
  induction' i with i hi
  · rw [partialSups_zero]; exact h 0
  rw [partialSups_succ]
  exact m.is_caratheodory_union hi (h (i + 1))

theorem isCaratheodory_disjointed {m : OuterMeasure α} {s : ℕ → Set α}
    (h : ∀ i, m.IsCaratheodory (s i)) (i : ℕ) : m.IsCaratheodory (disjointed s i) :=
  by
  induction' i with i hi
  · rw [disjointed_zero]; exact h 0
  rw [disjointed_succ, diff_eq]
  refine' m.is_caratheodory_inter (h (i + 1)) (m.is_caratheodory_compl _)
  exact is_caratheodory_partial_sups h i

theorem isCaratheodory_iUnion {m : OuterMeasure α} {s : ℕ → Set α}
    (h : ∀ i, m.IsCaratheodory (s i)) : m.IsCaratheodory (⋃ i, s i) :=
  by
  rw [← iUnion_disjointed]
  exact
    outer_measure.is_caratheodory_Union_nat m (is_caratheodory_disjointed h) (disjoint_disjointed _)
...
theorem isCaratheodory_generateFrom (hC : SetSemiring C) (m : ∀ s : Set α, s ∈ C → ℝ≥0∞)
    ...
    (m_sigma_subadd :
      ∀ ⦃f : ℕ → Set α⦄ (hf : ∀ i, f i ∈ C) (hf_Union : (⋃ i, f i) ∈ C),
        m (⋃ i, f i) hf_Union ≤ ∑' i, m (f i) (hf i))
    (s : Set α) (hs : measurable_set[MeasurableSpace.generateFrom C] s) :
    (inducedOuterMeasure m hC.empty_mem m_empty).IsCaratheodory s :=
  by
  refine' MeasurableSpace.generateFrom_induction _ C _ _ _ _ hs
  · exact fun _ => caratheodory_semiring_extension hC m m_empty m_add m_sigma_subadd
  · exact outer_measure.is_caratheodory_empty _
  · exact fun _ => outer_measure.is_caratheodory_compl _
  · exact fun _ hf => is_caratheodory_Union hf
...
noncomputable def Measure.ofAddSubadd [mα : MeasurableSpace α] (hC : SetSemiring C)
  ...
  (Measure.ofAddSubaddCaratheodory hC m m_empty m_add m_sigma_subadd).trim
    (by rw [← hC_gen]; exact is_caratheodory_generate_from hC m m_empty m_add m_sigma_subadd)
...
theorem Measure.ofAddSubadd_eq [mα : MeasurableSpace α] (hC : SetSemiring C)
    ...
    (m_sigma_subadd :
      ∀ ⦃f : ℕ → Set α⦄ (hf : ∀ i, f i ∈ C) (hf_Union : (⋃ i, f i) ∈ C),
        m (⋃ i, f i) hf_Union ≤ ∑' i, m (f i) (hf i))
    {s : Set α} (hs : s ∈ C) :
    Measure.ofAddSubadd hC hC_gen m m_empty m_add m_sigma_subadd s = m s hs :=
  by
  rw [measure.of_add_subadd, trim_measurable_set_eq]
  · exact measure.of_add_subadd_caratheodory_eq hC m m_empty m_add m_sigma_subadd hs
...
noncomputable def Measure.ofAddContent [mα : MeasurableSpace α] (hC : SetSemiring C)
  ...
  Measure.ofAddSubadd hC hC_gen (fun s _ => m s) addContent_empty
    (by
      intro I hI hI_disj hI_sUnion
      have h := m.add I hI hI_disj hI_sUnion
      refine' h.trans _
