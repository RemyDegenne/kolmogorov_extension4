...
theorem countably_additive_of_todo (hC : SetRing C) (m : ∀ s : Set α, s ∈ C → ℝ≥0∞)
  ...
  have hm_acc :
    ∀ (s : ℕ → Set α) (h_disj : Pairwise (Disjoint on s)) (h_meas : ∀ i, s i ∈ C) (n : ℕ),
    ...
    · simp only [le_zero_iff, Set.iUnion_iUnion_eq_left, Finset.range_one, Finset.sum_singleton]
  ...
  have h_tendsto : tendsto (fun n => m (s n) (hCs n)) at_top (𝓝 0) := hm hCs hs_anti hs_Inter
  have hmsn :
  ...
  have h_tendsto' :
    tendsto (fun n => ∑ i in Finset.range n, m (f i) (h i)) at_top (𝓝 (m (⋃ i, f i) hUf)) :=
    by
...
theorem countably_additive_addContent_of_todo (hC : SetRing C) (m : AddContent C)
    ...
    m (⋃ i, f i) = ∑' i, m (f i) :=
  countably_additive_of_todo hC (fun s _ => m s) (fun s hs => hm_ne_top hs)
    (fun s t => addContent_union m hC) hm hf hUf h_disj

theorem sUnion_eq_sum_of_union_eq_add (hC_empty : ∅ ∈ C)
    (hC_union : ∀ {s t : Set α} (hs : s ∈ C) (ht : t ∈ C), s ∪ t ∈ C) (m : Set α → ℝ≥0∞)
    (m_empty : m ∅ = 0)
    (m_add : ∀ {s t : Set α} (hs : s ∈ C) (ht : t ∈ C), Disjoint s t → m (s ∪ t) = m s + m t)
  ...
  have h_sUnion_mem : ⋃₀ ↑I ∈ C :=
    haveI : ∀ J : Finset (Set α), ↑J ⊆ C → ⋃₀ ↑J ∈ C :=
      by
      intro J
      induction' J using Finset.induction with s J hsJ h
...
theorem sUnion_eq_sum_of_union_eq_add' (hC_empty : ∅ ∈ C)
    (hC_union : ∀ {s t : Set α} (hs : s ∈ C) (ht : t ∈ C), s ∪ t ∈ C)
  ...
  by
  have h :=
    sUnion_eq_sum_of_union_eq_add hC_empty (fun s t hs ht => hC_union hs ht) (extend m)
